	...
	double solve() {
		...
		for (size_t itr = 0; itr < maxIterations; ++itr) {
	  	...
	  	lambda = ev[0];
			...
			update_stack_up(0);
			es_contract_root += double(std::clock() - start) / CLOCKS_PER_SEC;
			XERUS_LOG(simpleALS, "It:  " << itr << " CP " << 0  << " EV " << lambda -28.1930439210 << " Lerr =  "  << std::abs(lambda -28.1930439210	+38.979392539208));

			for (size_t corePosition : {1,3,4,2,5,6}) {
...
int main() {
  xerus::Index i1,i2,i3,i4, ii,jj,kk,ll;
	...
	auto dim = 26;
	double eps = 10e-7;
	std::vector<size_t> start_rank = std::vector<size_t>(2*dim-2,24);
	size_t max_rank = 12;
