...
public:
	...
	double solve() {
		...
		for (size_t itr = 0; itr < maxIterations; ++itr) {
	  	...
	  	if (op.dimensions[0] == 1 && op.dimensions[1] == 1 && op.dimensions[2] == 1 && op.dimensions[3] == 1){
			...
			for (size_t corePosition : {3,7,15,16,8,17,18,4,9,19,20,10,21,22,5,11,23,24,12,6,13,14}) {
				...
				if (Stack[pp].second.degree() == 6){ //TODO write special contraction for ARPACK Multiplication
					if (pc % 2 == 1)
						op (i1,i2,i3,i4,j1,j2,j3,j4) =  dummy(r1) * dummy(r2) * \
		  	...
		  	if (op.dimensions[0] == 1 && op.dimensions[1] == 1 && op.dimensions[2] == 1 && op.dimensions[3] == 1){
				...
				XERUS_LOG(simpleALS, "It:  " << itr << " CP " << corePosition  << " EV " << lambda -28.1930439210 << " Lerr =  "  << std::abs(lambda -28.1930439210	+38.979392539208));
