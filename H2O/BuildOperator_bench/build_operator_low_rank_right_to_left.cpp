...
class BuildingOperatorR2L{
	public:
		 ...
		 * Constructor
		 */
		BuildingOperatorR2L(size_t _d,std::string _path)
		: d(_d), path(_path){
		...
		TTOperator build_1_site_operator(std::queue<value_t> coeffs, const size_t dim, const bool transpose = false){
			...
			XERUS_REQUIRE(comp == 0, "comp needs to be 0, comp is " << comp);
			result.set_component(0,s);
...
int main(){
	...
	BuildingOperatorR2L builder(d,path);
	...
	xerus::TTOperator op;
	std::string name = "../hamiltonian_H2O_48_full_3.ttoperator";
	...
	name = "../hamiltonian_H2O_48_full_R.ttoperator";
