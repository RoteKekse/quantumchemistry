...
public:
  ...
  void push_left_stack(const size_t _position,const TTTensor& y) {
	...
	void push_left_stack(const size_t _position,const TTTensor& y,const TTOperator& A,const TTOperator& F) {
    ...
    const Tensor &yi = y.get_component(_position);
		...
		Tensor tmpA;
		tmpA(i1, i2, i3, i4, i5) = leftYStack.back()(j1, j2, j3,j4,j5)
				*xi(j1, k1, i1)*Fi(j2,k1,k2,i2)*Ai(j3, k2, k3, i3)*Fi(j4,k3,k4,i4)*yi(j5, k4, i5);
		leftYStack.emplace_back(std::move(tmpA));
	...
	void push_right_stack(const size_t _position,const TTTensor& y,const TTOperator& A,const TTOperator& F) {
    Index i1, i2, i3,i4,i5, j1 , j2, j3, j4, j5, k1, k2,k3,k4;
		const Tensor &xi = xbasis[0].get_component(_position);
		const Tensor &Ai = A.get_component(_position);
		const Tensor &Fi = F.get_component(_position);
		...
		Tensor tmpA;
		tmpA(i1, i2, i3, i4, i5) = xi(i1, k1, j1)*Fi(i2,k1,k2,j2)*Ai(i3, k2, k3, j3)*Fi(i4,k3,k4,j4)*yi(i5, k4, j5)
				*rightYStack.back()(j1, j2, j3,j4,j5);
		rightYStack.emplace_back(std::move(tmpA));
	}
  ...
  std::vector<Tensor> getlocalVector(const TTTensor& y){
    ...
    for (size_t corePosition=0;corePosition<d;corePosition++){
      Tensor rhs;
			...
			rhs(i1, i2, i3) = leftYStack.back()(i1, k1) * yi(k1, i2, k2) * rightYStack.back()(i3, k2);
  ...
  std::vector<Tensor> localProduct(const std::vector<Tensor>& y,const TTOperator& A,const TTOperator& F){
		TTTensor Y=builtTTTensor(y);
		return localProduct(Y, A,F);
  }

  std::vector<Tensor> localProduct(const std::vector<Tensor>& y,const TTOperator& A){
		TTOperator F = TTOperator::identity(A.dimensions);
		return localProduct(y,A,F);
  }

  std::vector<Tensor> localProduct(const TTOperator& A){
		TTOperator F = TTOperator::identity(A.dimensions);
		return localProduct(A,F);
  }

  std::vector<Tensor> localProduct(const TTOperator& A, const TTOperator& F){
		TTTensor Y=xbasis[0];
		return localProduct(Y, A,F);
  }

  std::vector<Tensor> localProduct(TTTensor& Y,const TTOperator& A, const TTOperator& F){
		std::vector<Tensor> locY;
    leftYStack.clear();
		rightYStack.clear();

    leftYStack.emplace_back(Tensor::ones({1,1,1,1,1}));
		rightYStack.emplace_back(Tensor::ones({1,1,1,1,1}));
       
		for (size_t pos=d-1; pos>0;pos--){
    	push_right_stack(pos,Y,A,F);
		}
		for (size_t corePosition=0;corePosition<d;corePosition++){
      Tensor rhs;
			const Tensor &yi = Y.get_component(corePosition);
			...
			rhs(i1, i2, i3) = leftYStack.back()(i1,A1,A2,A3, k1) * Fi(A1,i2,j1,A4) * Ai(A2,j1,j2,A5)* Fi(A3,j2,j3,A6) *yi(k1, j3, k2) * rightYStack.back()(i3,A4,A5,A6, k2);

			locY.emplace_back(rhs);

			if (corePosition+1 < d) {
            
				push_left_stack(corePosition,Y,A,F);
				rightYStack.pop_back();
      }
		}
		Index i,j;
  ...
  void krylow(const TTOperator& A, const TTOperator& B,const TTOperator& F,const TTOperator& FF,const std::vector<Tensor>& x0,const std::vector<Tensor>& rhs,std::vector<Tensor>& x, size_t maxit, double error){
    //Operator is A - B
		time_t begin_time = time (NULL);
		bool cont=true;
		...
		while((it<maxit)&&(resnorm2>error*rhsnorm*rhsnorm)&&cont){
			auto tmp=localProduct(v,A,F);
			auto tmpB=localProduct(v,B);
			auto tmpFF=localProduct(x0,FF);

			add(tmp,tmpB);
			project(tmp,x0,tmpFF); ///TODO add FF so it is (I-Fxx^T) Ax, check if project method is correct
    ...
    XERUS_LOG(info,"krylow Ax-b: "<<sqrt(resnorm2)/rhsnorm<<"   time for krylow method: "<<time (NULL)-begin_time<<" sekunden  "<<it<<"  iterationen");
