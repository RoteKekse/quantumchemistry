...
#pragma omp parallel for
		...
		void updateResult(const size_t dim){
			...
			//first summand
			result = addNextSite(result, current_wave_function.get_component(k),k);
			if (active_site){
				auto tmpQ1 = addNextSite(Q1[k][k],e1,k);
				auto tmpS2 = addNextSite(S2[k],e0,k); //TODO check why is S2 0?
				auto tmpk = addNextSite(current_wave_function,T[{k,k}]*e1,k);

				result += tmpk + sign*tmpS2 +  tmpQ1;


			} else {
				auto tmpS1 = addNextSite(S1[k],e1,k);
				result -= sign*tmpS1;
			}
			result.round(0.0);
		}
		...
		void update3SiteOperatorsS(const size_t l, const size_t dim){
			...
			Tensor e0 = Tensor::dirac({1,2,1},{0,0,0});
			Tensor e1 = Tensor::dirac({1,2,1},{0,1,0});
			S1[l] = addNextSite(S1[l],current_wave_function.get_component(k),k);
			S2[l] = addNextSite(S2[l],current_wave_function.get_component(k),k);
...
//				tests1(coeffs2,k,true);
				S1[l] += sign*addNextSite(current_wave_function,T[{l,k}]*e0,k) + sign*addNextSite(Q1[l][k],e0,k) + addNextSite(build1SiteTensor(coeffs1, k, false),e1,k);
				S2[l] += sign*addNextSite(P2[l][k],e0,k) ;
				S2[l] += addNextSite(build1SiteTensor(coeffs2, k, true),e1,k);
			} else {
				S1[l] += sign*addNextSite(P1[l][k],e1,k); //TODO check P1[k][l] or P1[l][k]
				S2[l] += sign*addNextSite(current_wave_function,T[{l,k}]*e1,k) +  sign*addNextSite(Q2[l][k],e1,k);
			}
			if (k%25 == 0){
				S1[l].round(0.0);
				S2[l].round(0.0);
...
//			S[s] = S_tmp + S_tmp2 - S_tmpP + S_tmpQ - tmp;
		...
		void update2SiteOperatorQ(const size_t l, const size_t m, const size_t dim){// TODO check if Q1 and Q2 are definitively different
			...
			time_t begin_time = time (NULL);
			Q1[l][m] = addNextSite(Q1[l][m],current_wave_function.get_component(k),k);
			update_Q1ext += time (NULL) -  begin_time;
			begin_time = time (NULL);
			Q2[l][m] = addNextSite(Q2[l][m],current_wave_function.get_component(k),k);
...
//				tests1(coeffs2,k,true);
				...
				begin_time = time (NULL);
				Q1[l][m] += addNextSite(current_wave_function,val*e1,k) + sign*addNextSite(build1SiteTensor(coeffs1, dim-1, true),e0,k);
				update_Q1add += time (NULL) -  begin_time;
				begin_time = time (NULL);
				Q2[l][m] += addNextSite(current_wave_function,val*e1,k) + sign*addNextSite(build1SiteTensor(coeffs2, dim-1, true),e0,k);
...
//				tests1(coeffs2,k,false);
				begin_time = time (NULL);
				Q1[l][m] -=  sign*addNextSite(build1SiteTensor(coeffs1, dim-1, false),e1,k);
				update_Q1add += time (NULL) -  begin_time;
				begin_time = time (NULL);
				Q2[l][m] -=  sign*addNextSite(build1SiteTensor(coeffs2, dim-1, false),e1,k);
				update_Q2add += time (NULL) -  begin_time;
...
//
		...
		void update2SiteOperatorsP(const size_t l, const size_t m, const size_t dim){
			...
			P1[l][m] = addNextSite(P1[l][m],current_wave_function.get_component(k),k);
			P2[l][m] = addNextSite(P2[l][m],current_wave_function.get_component(k),k);
...
//				tests1(coeffs1,k,false);
				P1[l][m] -= sign*addNextSite(build1SiteTensor(coeffs1, dim-1, false),e0,k);
			} else {
				for (size_t s = 0; s < k; ++s){
					coeffs1.push(V[{k,s,l,m}] - V[{k,s,m,l}]);
				}
//				tests1(coeffs1,k,true);
				P2[l][m] -= sign*addNextSite(build1SiteTensor(coeffs1, dim-1, true),e1,k);
			}
			if (k%25 == 0){
				P1[l][m].round(0.0);
				P2[l][m].round(0.0);
...
//			P[r][s] = P_tmp + tmp;
		...
		TTTensor addNextSite(TTTensor tensor_train, Tensor next_comp,size_t k){
			time_t begin_time = time (NULL);
