...
class BuildingPartialOperatorL2R{
	public:
		size_t d; //number of basis functions
		std::vector<size_t> partial; // list of number of basis functions for which the operator is supposed to be build partially
		Tensor V; // 2 electron integrals as order 4 Tensor
		Tensor T; // 1 electron integrals as order 2 Tensor
		Tensor N; // nuclear potential
		std::string path_T; // path to T
		std::string path_V; // path to V
		value_t nuc; // value of N
		std::vector<std::vector<TTOperator>> P; // P Stack of the recursion
		std::vector<std::vector<TTOperator>> Q; // Q Stack of the recursion
		std::vector<TTOperator> S;              // S Stack of the recursion
		TTOperator H;														// current recursive Hamiltonian
		std::vector<TTOperator> Hlist;              // list of Partial operators which are built recursively, one for each element in the list partial

		/*
		 * Constructor
		 */
		BuildingPartialOperatorL2R(size_t _d, std::vector<size_t> _partial, std::string _path_T, std::string _path_V, value_t _nuc)
		: d(_d), path_T(_path_T),path_V(_path_V), nuc(_nuc), partial(_partial){
		...
		void build(){
			...
			if(std::binary_search (partial.begin(), partial.end(), 0))
				Hlist.emplace_back(H);
				//Hlist.emplace_back(build_partial_operator(0));
			auto max = std::max_element(partial.begin(),partial.end());
			XERUS_LOG(info,"max = " << *max);
		 ...
		 */
		void step(const size_t k, const size_t max){
			XERUS_LOG(BuildOpL2R, "- Step " << k);
			update_4_site_operator_H(k+1);

			if(std::binary_search (partial.begin(), partial.end(), k+1))
				Hlist.emplace_back(H);
				//Hlist.emplace_back(build_partial_operator(k+1));
#pragma omp parallel for
			for (size_t s = k; s < max; ++s)
				update_3_site_operator_S(s,k+1);

#pragma omp parallel for
			for (size_t r = k; r < max; ++r){
				for (size_t s = k; s < max; ++s){
		...
		/*
		 * adds the missing 1 and 2 site interaction to the operator which are not highdimensional,
		 * i.e. t_ij and v_ijij and v_ijji
		 */
		TTOperator build_partial_operator(size_t dtilde){
			XERUS_REQUIRE(dtilde <= d, "Dtilde not smaller than d");
			XERUS_LOG(info, "Build Partial Operator for " << dtilde << " basis functions");
			TTOperator opPartial;
			if (dtilde > 0){
				opPartial = H;
				XERUS_LOG(info,opPartial.order());
				for (size_t i = dtilde; i < d; ++i){
					opPartial = add_identity(opPartial);
				}
			}
			else
				opPartial = xerus::TTOperator(std::vector<size_t>(2*d,2));
			XERUS_LOG(info,opPartial.order());
			auto opH = xerus::TTOperator(std::vector<size_t>(2*d,2));
			for (size_t i = 0; i < d; i++){
				XERUS_LOG(info,i);
				for (size_t j = 0; j < d; j++){
					if(i >= dtilde || j >= dtilde){
						value_t val = T[{i , j}];
			...
			auto opV = xerus::TTOperator(std::vector<size_t>(2*d,2));
			for (size_t i = 0; i < d; i++){
				XERUS_LOG(info,i);
				for (size_t j = 0; j < d; j++){
					if ((i >= dtilde || j >= dtilde ) && (i != j )){
						value_t val = V[{i,j,i,j}] - V[{i,j,j,i}];
						if (std::abs(val) < 10e-14)
							continue;
						opV += 0.5*val * return_two_e_ac(i,j,j,i,d); //TODO test for 0.5 and 1.0 as factor
					}
				}
				opV.round(0.0);
			}
			XERUS_LOG(info,opH.ranks());
			XERUS_LOG(info,opV.ranks());

			opPartial += opV + opH;
			opPartial.round(0.0);
			XERUS_LOG(info,opPartial.ranks());
