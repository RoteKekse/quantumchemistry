...
int main(){
	srand(time(NULL));
	size_t d = 48, p = 8, position = 5, iterations = 1e6;
	bool proj = true;
	...
	Tensor test_component = get_test_component(position,phi,ev_exact, proj);
  ...
  Metropolis<ProjectorProbabilityFunction> markow2(&PPF, TrialSample2, sample, d);
  unitVectorProjection uvP(phi,position);

//  sample2 = { 0, 1, 20, 21, 22, 23, 30, 31 };
//  XERUS_LOG(info, sample2 << " " << PPF.P(sample2) << "\n" << uvP.localProduct(sample2,proj));
//  sample2 = { 0, 1, 2, 3, 22, 23, 30, 31 };
//  XERUS_LOG(info, sample2 << " " << PPF.P(sample2) << "\n" << uvP.localProduct(sample2,proj));
//  sample2 = { 0, 1, 2, 20, 22, 23, 30, 31 };
//  XERUS_LOG(info, sample2 << " " << PPF.P(sample2) << "\n" << uvP.localProduct(sample2,proj));
//  sample2 = { 0, 1, 3, 20, 22, 23, 30, 31 };
//	XERUS_LOG(info, sample2 << " " << PPF.P(sample2) << "\n" << uvP.localProduct(sample2,proj));
//	sample2 = { 0,  2,3, 20, 22, 23, 30, 31 };
//	XERUS_LOG(info, sample2 << " " << PPF.P(sample2) << "\n" << uvP.localProduct(sample2,proj));
//	sample2 = { 1,  2,3, 20, 22, 23, 30, 31 };
//	XERUS_LOG(info, sample2 << " " << PPF.P(sample2) << "\n" << uvP.localProduct(sample2,proj));
//
  ...
  runMetropolis<ProjectorProbabilityFunction>(&markow2,umap2,iterations);
	...
	XERUS_LOG(info,"projected ?  "<< proj);
	XERUS_LOG(info,"test component: "<< test_component.frob_norm() << "\n" << test_component);
	XERUS_LOG(info,"result*prob:    "<< (result*prob).frob_norm() << "\n" << result*prob);
	XERUS_LOG(info,"test component - result*prob (abs):    "<< (test_component-result*prob).frob_norm() << "\n" << test_component-result*prob);
	XERUS_LOG(info,"test component - result*prob (rel):    "<< (test_component-result*prob).frob_norm() / test_component.frob_norm()<< "\n" << test_component-result*prob);
