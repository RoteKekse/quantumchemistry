...
public:
		...
		: d(_x.order()), x(_x), A(_A), M(_M), lambda(_l), b(_b), maxIterations(1)
	{
		...
		leftMStack.emplace_back(Tensor::ones({1,1,1}));
		rightMStack.emplace_back(Tensor::ones({1,1,1}));
	}
	...
	void push_left_stack(const size_t _position) {
		...
		tmpM(i1, i2, i3) = leftMStack.back()(j1, j2, j3)
				*xi(j1, k1, i1) * Mi(j2,k1,k2,i2)*bi(j3, k2, i3);
	...
	void push_right_stack(const size_t _position) {
		...
		Tensor tmpA, tmpM;
		tmpA(i1, i2, i3) = xi(i1, k1, j1)*Ai(i2,k1,k2,j2)*bi(i3, k2, j3)
				*rightAStack.back()(j1, j2, j3);
		rightAStack.emplace_back(std::move(tmpA));
		tmpM(i1, i2, i3) = xi(i1, k1, j1)*Mi(i2,k1,k2,j2)*bi(i3, k2, j3)
				*rightMStack.back()(j1, j2, j3);
	...
	void solve() {
		...
		for (size_t itr = 0; itr < maxIterations; ++itr) {
			...
			for (size_t corePosition = 0; corePosition < d; ++corePosition) {
				...
				rhsA(i1, i2, i3) =   leftAStack.back()(i1, k1, k2) * Ai(k1,i2,j2,l1)* bi(k2, j2, l2) *   rightAStack.back()(i3, l1,l2);
				rhsM(i1, i2, i3) =   leftMStack.back()(i1, k1, k2) * Mi(k1,i2,j2,l1)* bi(k2, j2, l2) *   rightMStack.back()(i3, l1,l2);
