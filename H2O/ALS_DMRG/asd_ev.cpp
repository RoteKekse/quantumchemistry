...
public:
	...
	double solve() {
		...
		for (size_t itr = 0; itr < maxIterations; ++itr) {
			...
			XERUS_LOG(simpleALS, "Iteration: " << itr  <<  " EV " << std::setprecision(10) <<  lambda + 8.80146457125193 << " EV2 = " << lambda << " err = " << std::abs(lambda + 8.80146457125193 + 76.256624));// << " Residual = " << calc_residual_norm());
