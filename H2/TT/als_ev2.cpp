...
//
//	xerus::Tensor perm_T = make_Permutation(perm, nob);
//
//  xerus::Index i1,i2,i3,i4, ii,jj,kk,ll;
//
//  H(i1,i2) = perm_T(i1,jj) * H(jj,kk) *perm_T(i2,kk);
#if build_operator
  //xerus::Tensor V = make_V(nob);
  xerus::Tensor V = make_V_BE();
  V(i1,i2,i3,i4) = perm_T(i1,ii) * perm_T(i2,jj) * perm_T(i3,kk) * perm_T(i4,ll) * V(ii,jj,kk,ll);
  xerus::TTOperator operatorV = return_op_V(V);
#else
//	auto operatorV = xerus::TTOperator(std::vector<size_t>(2*d,2)); // TT operator initialized with 0
//
...
//  	std::string name = "hamiltonian"+std::to_string(d)+"_" +std::to_string(i)+ ".ttoperator";
...
//  xerus::TTOperator op = return_op_H(H) + operatorV;
//
...
//  write.close();
	xerus::TTOperator op;
	std::string name = "hamiltonian" + std::to_string(d) +"_full.ttoperator";
	std::ifstream read(name.c_str());
...
xerus::Tensor make_H_BE(){
  ...
  H[{1,2}] =   		0.2190509890;
  H[{2,2}] =      -1.594069613;
	H[{3,1}] =    -0.5038662979E-01;
	H[{1,3}] =    -0.5038662979E-01;
	H[{3,2}] =     0.1590245167;
	H[{2,3}] =     0.1590245167;
	H[{3,3}] =    -0.4996472436;
	H[{4,1}] =     0.2158425412;
