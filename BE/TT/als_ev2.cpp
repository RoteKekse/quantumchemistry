...
int main() {
	...
	size_t d = 16; // 16 electron, 8 electron pairs
	size_t nob = 8;
	double eps = 10e-12;
	size_t start_rank = 2;
	size_t max_rank = 16;
		...
		for (size_t itr = 0; itr < maxIterations; ++itr) {
  	  	...
  	  	std::unique_ptr<double[]> ev(new double[1]);      // real eigenvalues
		  	if (xi.dimensions[2] > 32)
		  		xerus::get_smallest_eigenvalue_iterative_dmrg_special(sol,leftAStack.back(),Ai,Ai1,rightAStack.back(),ev.get(), 1, 100000, 10e-12);
		  	else {
					op(i1, i2, i3, i4, j1, j2, j3, j4) = leftAStack.back()(i1, k1, j1)*Ai(k1, i2, j2, k2) * Ai1(k2,i3,j3,k3)*rightAStack.back()(i4, k3, j4);
					xerus::get_smallest_eigenvalue_iterative(sol,op,ev.get(), 1, 100000, 10e-12);
		  	}
		  	//XERUS_LOG(info,sol);
		  	lambda = ev[0];
...
//				}
			...
			XERUS_LOG(simpleALS, "Iteration: " << itr  <<  " Eigenvalue " << std::setprecision(10) <<  lambda <<  " EVerr " << lambda + 14.37016558404629);// << " Residual = " << calc_residual_norm());
