...
int main() {
  ...
  xerus::TTOperator op = opH + opV;
	...
	//xerus::HTTensor phi = xerus::HTTensor::random(std::vector<size_t>(dim,2),{7,8,3,4,5,6,2,2,2,2,2,2,2,2});
	double lambda = simpleMALS(op, phi, eps, max_rank);//
	XERUS_LOG(info, "The ranks of phi are " << phi.ranks() << " Norm = " << frob_norm(phi));
