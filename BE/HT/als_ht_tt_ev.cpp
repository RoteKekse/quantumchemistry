...
public:
	...
	InternalSolver(const TTOperator& _A, HTTensor& _x)
		: d(_x.degree()), x(_x), A(_A),  maxIterations(1000), p(static_cast<size_t>(std::log(_x.degree()))), lambda(1.0)
	{
	}
...
//
		for (size_t itr = 0; itr < maxIterations; ++itr) {
			...
			for (size_t corePosition : trav) {
				...
				lambda = xerus::get_smallest_eigenvalue(xi, op);
				if (corePosition == 0) {
...
double simpleALS(const TTOperator& _A, HTTensor& _x)  {
