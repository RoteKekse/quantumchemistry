...
public:
	...
	void initizialize_stack(){
		...
		// then the internal components till the root edges
		for (int i = d - 3; i >= 0; --i){
			auto htt_comp_idx = i + 1;
		...
		// no go up from the root to the leaves
		// first the root
		{
		x.move_core(1);
		auto xi = x.get_component(0);
		...
		// then the inner components
		for (size_t i = 2; i < d/2 -2; i=i+2){
			auto htt_comp_idx = i / 2;
			...
			x.move_core(i+1);
			auto xi = x.get_component(htt_comp_idx);
			if (std::abs(std::log2(i+2) - std::floor(std::log2(i+2))) < 10e-12){
				tmp1(i1,i2,i3,i4) = parent(r1,j3,i3,r2) * xi(r1,i1,j2) * xi(r2,i4,j4)  * rchild(j2,i2,j3,j4);
			} else if (std::abs(std::log2(i+4) - std::floor(std::log2(i+4))) < 10e-12){
			...
			Stack[i].second = tmp1;
			x.move_core(i+2);
			xi = x.get_component(htt_comp_idx);
			if (std::abs(std::log2(i+4) - std::floor(std::log2(i+4))) < 10e-12){
				tmp2(i1,i2,i3,i4) = parent(r1,i2,j3,r2) * xi(r1,j2,i1) * xi(r2,j4,i4)  * lchild(j2,j3,i3,j4);
			} else if (std::abs(std::log2(i+2) - std::floor(std::log2(i+2))) < 10e-12){
			...
			Stack[i+1].second = tmp2;
		}
	}

	void update_stack_internal(size_t _child){
		Index i1, i2, i3,i4, i5, i6, j1 , j2, j3,j4, k1, k2,r1,r2;
		XERUS_REQUIRE(_child > 2,"_child includes root component, use update_stack_root instead");
		size_t firstchild = _child % 2; // 1 means left child 0 means right child
		x.move_core(0);
		for (size_t i: {_child - 1, (_child - 1)/2 - 1}){
			auto htt_comp_idx = i + 1;
		...
		//then _child - 1 second
		for (size_t i: {2*((_child - 1)/2),2*_child}){
			if (i >= d/2 - 2)
				continue;
			auto htt_comp_idx = i / 2;
			auto lchild = Stack[2*htt_comp_idx].first;
	...
	double solve() {
		...
		for (size_t itr = 0; itr < maxIterations; ++itr) {
			...
			for (size_t corePosition : {14,13,6,12,11,5,10,9,4,8,7,3}) {
				...
				for( size_t i = 0; i < 2*d - 2; i++ )
				{
					XERUS_LOG(info,"i = " << i);
					XERUS_LOG(info,"\n" << x.get_component(i));
					XERUS_LOG(info,"\n" << x_bet.get_component(i));
				}
				return 0;
				Tensor op,s1,s2,s3,s4;
				...
				TensorNetwork optn1,optn2,optn3,optn4;
//				XERUS_LOG(info,"corePosition: " << corePosition);
//
//				optn1() =   x(i1^d)*x(j1^d) *  A(i1^d,j1^d);
//				for ( size_t i = 0; i < optn1.nodes.size(); i++){
//					if (i != 30 && i !=62 && i != 80 ){
//						optn1.nodes[i].erase();
//					}
//				}
//				optn1.sanitize();
//				optn1.draw("optn1");
//				s1(i1,i3,i4,i2) = optn1(i1,i2,i3,i4);
//				XERUS_LOG(info,"op - op1 = " << (s1-Stack[rc].first).frob_norm());
//				optn2() =   x(i1^d)*x(j1^d) *  A(i1^d,j1^d);
//				for ( size_t i = 0; i < optn2.nodes.size(); i++){
//					if (i != 29 && i !=61 && i != 79 ){
//						optn2.nodes[i].erase();
//					}
//				}
...
//				optn2.draw("optn2");
//
//				s2(i1,i3,i4,i2) = optn2(i1,i2,i3,i4);
//				XERUS_LOG(info, "\n" << *(optn2.nodes[0].tensorObject));
//				XERUS_LOG(info, "\n" << x.get_component(29));
//				XERUS_LOG(info,"op - op2 = " << (s2-Stack[lc].first).frob_norm());
//				return 0;
				//optn.draw("optn" + std::to_string(S[{i,i}]) );
...
//				XERUS_LOG(info,"x  ranks = " << x.ranks());
//				XERUS_LOG(info,"S  ranks = " << S.dimensions);
//				XERUS_LOG(info,"U  ranks = " << U.dimensions);
//				XERUS_LOG(info,"Vt  ranks = " << Vt.dimensions);
				...
				for (size_t i = 0; i < d - 1; ++i){
					if (i != (corePosition-1)/2) {
						//XERUS_LOG(info,"i: " << i);

						auto xtmp = x.get_component(i);
						Tensor tmp1,tmp2, tmp3;
						tmp1(i1,j1) = xtmp(i1,r1,r2) * xtmp(j1,r1,r2);
						tmp2(i1,j1) = xtmp(r1,i1,r2) * xtmp(r1,j1,r2);
						tmp3(i1,j1) = xtmp(r1,r2,i1) * xtmp(r1,r2,j1);
//						XERUS_LOG(info,"\n"<< tmp1);
//						XERUS_LOG(info,"\n"<< tmp2);
//						XERUS_LOG(info,"\n"<< tmp3);
				...
				x_bet = x;
				start = std::clock();
				update_stack_internal(corePosition);
				es_contract_inner += double(std::clock() - start) / CLOCKS_PER_SEC;
			...
			Tensor sol; //TODO take old component!
...
//
			...
			XERUS_LOG(simpleALS, "It:  " << itr << " CP: " << 0 << "\tRes: " << calc_residual_norm() << " EV " << lambda);
